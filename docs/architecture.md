...
| `Constraints`/`Objective` | Pyomo math (expressions) | One `Rules` class per problem, one **named method** per constraint/objective; both split attach/activate into separate single-purpose methods — building is always unconditional, activation (static `enabled`/`profile`, or ad-hoc by name) is a distinct call (see [Configuration reference](configuration.md)) |
